...
class ConvAtPointMultiOut_SASpec extends AnyFreeSpec with Matchers {

  "ConvAtPointMultiOut_SA should match rc.py y[m=0..2, oh=0, ow=0]" in {
    val cout = 3
    val cin  = 2
    val h    = 5
    val w    = 5
    ...
    val expectedY = Seq(7, 8, 5)

    simulate(new ConvAtPointMultiOut_SA(cout, cin, h, w, kh, kw, cfg, flushExtra = 6)) { dut =>
      // poke input
      ...
      var cycles = 0
      var done = false
      while (cycles < 2000 && !done) {
        done = dut.io.done.peek().litToBoolean
        dut.clock.step()
        cycles += 1
      }
      withClue("should finish within 2000 cycles: ") {
        done mustBe true
      }
